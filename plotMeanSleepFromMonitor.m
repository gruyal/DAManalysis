function plotMeanSleepFromMonitor(damTable, monitorNum, varargin)

% function plotSleepMonitor(damTable, monitorNum)
%
% Plots sleep by day on ZT for the data from one activity monitor
%
% INPUT
%
...
%tiledlayout(length(daysToPlot)-1, 1, 'TileSpacing','none')
...
for mm=1:length(monitorNum)
    ...
    for day=1:length(daysToPlot)-1
        ...
        hold on
        meanSleep = tempTab.sleepProp;
        smMeanData = smooth(meanSleep, smWin, 'rloess');
...
lgH = legend(axh(end),flipud(axh(end).Children(1:end-2)), legTit);
legend('boxoff')

lgH.Layout.Tile = 'south';
lgH.Orientation = "horizontal";
